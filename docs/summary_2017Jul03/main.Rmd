...
1. **Are mean coverages sensible?** Inferred autosomal, mitochondrial and rDNA coverages are consistent with estimates from e.g. QualiMap. QualiMap's estimated mean coverages are slightly smaller, due to the inclusion of zero coverage regions.

2. **Why the large differences between inferred CNs from the full and subsampled data?** It is not entirely clear to me why this is the case, and whether subsampling is a valid approach to explore the robustness of CN inference. For example, subsampling may introduce significant changes in mean coverages, and despite the method itself being robust, estimated CNs may differ significantly.

3. **Do standard errors make sense?** Shown standard errors capture the technical (statistical) uncertainty of the inference method. Biological variability should be the main source of variability. Since there are no replicates, I cannot comment on the robustness of the estimates.
