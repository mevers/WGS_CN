...
<div style="font-size:80%">
$$
se(\text{CN}) = CN \times \sqrt{\left(\frac{\text{sem coverage(rDNA)}}{\text{mean coverage(rDNA)}}\right)^2 + \left(\frac{\text{sem coverage(diploid DNA)}}{\text{mean coverage(diploid DNA)}}\right)^2} 
$$
</div>
...
<li> We use a sliding window approach to estimate sequencing depth (coverage) across (1) the diploid DNA, (2) the rDNA repeat, and (3) the mitochondrial DNA. A sliding window approach is advantegeous because it will cause smoothing of regions with large deviation in coverage from uniformity. Here we use a window of width $w = 10\text{ kb}$ and a step size of $s = 1\text{ kb}$.
</ol>
